...
import { deriveStats, isMaxed, upgradeCost } from './economy';
import {
  SAVE_VERSION,
  type AdvanceResult,
  type GameState,
 ...
 *     elapsed seconds, computed in O(1) regardless of how big the delta is.
 *
...
export function createNewGame(
  ...
  return {
    ...
    lifetimeEarnings: 0,
    upgrades: { cranes: 0, shipSize: 0, contracts: 0 },
    berthCycleSeconds: 0,
    lastTickAt: now,
  };
}
 ...
 *
 * Earnings across the delta are computed exactly, in closed form:
 *   ships = fullCyclesCrossed - fractionAt(start) + fractionAt(end)
 * so a 4-hour delta costs the same as a 100ms one and loses no precision to
...
export function advance(
  ...
  const stats = deriveStats(state, cfg);
...
/**
 * Buy one level of an upgrade. Returns the original state untouched if the
 * player cannot afford it or the track is maxed, so callers can just try.
 */
export function buyUpgrade(
  ...
  if (isMaxed(id, level, cfg)) return { state, bought: false };
...
/**
 * Coerce anything loaded from disk into a valid GameState. Saves are the one
 * place untrusted data enters the sim, and a single NaN here would poison
 * every subsequent tick, so every field is checked.
 */
export function sanitise(
  ...
  const level = (v: unknown, id: UpgradeId): number => {
    const n = Math.floor(num(v, 0));
    const max = cfg.upgrades[id].maxLevel;
    const capped = max === null ? n : Math.min(n, max);
  ...
  for (const id of UPGRADE_ORDER) {
    upgrades[id] = level(rawUpgrades[id], id);
  }
  ...
  return {
    ...
    upgrades,
    berthCycleSeconds: Math.max(0, num(r.berthCycleSeconds, 0)),
